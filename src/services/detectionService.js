...
class DetectionService {
    ...
    async processSearch(searchTerm) {
        try {
            // PASO 0: Buscar en caché (Mongo y Sheets)
            const existingData = await this.searchInBothSources(searchTerm);
            if (existingData) {
                console.log('✅ Datos encontrados en caché');
                return { success: true, source: 'cached', data: existingData };
            }
            
            // PASO 1: Detectar DUTY con GROQ
            const dutyAnalysis = await groqService.detectDuty(searchTerm);
            console.log(`🤖 DUTY detectado: ${dutyAnalysis.duty}`);
            
            // PASO 2: Scraping según DUTY
            ...
            // PASO 3: Construir array de productos (TRILOGY)
            const products = [];
            ...
            // PASO 4: Generar SKUs para cada producto
            ...
            // PASO 5: Guardar en MongoDB y Sheets
            await Promise.all([
            ...
            // PASO 6: Preparar respuesta para el frontend
