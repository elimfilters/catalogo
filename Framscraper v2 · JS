...
/**
 * FRAM Scraper - ELIMFILTERS v2.0
 * Scraper avanzado con extracción completa de especificaciones técnicas
 * ESPEJO EXACTO de donaldsonScraper para aplicaciones Light Duty (LD)
 */

const framScraper = {
    /**
     * Busca cross-references en FRAM con extracción completa de specs
     * @param {string} searchTerm - Código original (ej: FL820S)
     * @returns {Array} Array de hasta 3 cross-references con specs completas
     */
    search: async (searchTerm) => {
        try {
            console.log(`🔍 FRAM Scraper v2.0: Searching for ${searchTerm}`);
            
            // PASO 1: Búsqueda Inicial
            const searchUrl = `https://www.fram.com/search-results?q=${encodeURIComponent(searchTerm)}`;
            ...
            if (!productPath) {
                ...
                return framScraper.getFallbackResults(searchTerm);
            }
            
            // PASO 2: Navegación a Ficha Técnica
            const productUrl = productPath.startsWith('http') 
                ? productPath 
                : `https://www.fram.com${productPath}`;
            ...
            const techSpecs = {};
            $('.product-attributes-table tr, .attributes-table tr, .specifications-table tr, .specs-table tr').each((i, row) => {
            ...
            $('.alternative-products-list .product-number, .related-products .part-number, .cross-reference-item').each((i, el) => {
                ...
                if (altCode && altCode !== baseCode && results.length < 3) {
                    const isElite = altCode.startsWith('XG') || 
                                  altCode.includes('ULTRA') || 
                                  altCode.includes('SYNTHETIC');
                    
                    const tier = isElite ? 'ELITE' : 'PERFORMANCE';
                    const microns = isElite ? 15 : 21;
                    const mediaType = isElite ? 'Synthetic (FRAM Ultra)' : 'Celulosa Enhanced (Extra Guard)';
            ...
            if (results.length === 1) {
                results.push(...framScraper.generateAlternatives(baseCode, rules, systemKey, techSpecs));
            }
            
            console.log(`✅ FRAM Scraper: Found ${results.length} results with complete specs`);
            return results;
        ...
        return [
            {
                ...
                microns: 21,
                mediaType: 'Celulosa Enhanced (Extra Guard)',
                tier: 'PERFORMANCE',
            ...
            },
            {
                ...
                microns: 15,
                mediaType: 'Synthetic (FRAM Ultra)',
                tier: 'ELITE',
                source: 'Generated Pattern',
                specs: techSpecs,
                systemKey: systemKey,
    ...
    getFallbackResults: (searchTerm) => {
        ...
        const baseNum = numMatch[0];
        const paddedNum = baseNum.slice(-4).padStart(4, '0');
        
        return [
            {
                code: `PH${paddedNum}`,
                description: 'FRAM Extra Guard - Performance',
                microns: 21,
                mediaType: 'Celulosa Enhanced (Extra Guard)',
                tier: 'PERFORMANCE',
            ...
            {
                code: `CH${paddedNum}`,
                description: 'FRAM Tough Guard - Standard',
                microns: 40,
                mediaType: 'Celulosa Standard',
                tier: 'STANDARD',
                source: 'Fallback Pattern',
            ...
            {
                code: `XG${paddedNum}`,
                description: 'FRAM Ultra Synthetic - Elite',
                microns: 15,
                mediaType: 'Synthetic (FRAM Ultra)',
                tier: 'ELITE',
                source: 'Fallback Pattern',
    ...
    /**
     * Obtiene aplicaciones de vehículos desde la página de producto
     */
    getVehicleApplications: async (productCode) => {
        try {
            const detailUrl = `https://www.fram.com/products/${productCode.toLowerCase()}`;
            ...
            const applications = [];
            $('.fits-vehicle, .vehicle-application, .compatible-vehicle, .application-item').each((i, el) => {
            ...
            return applications.length > 0 
                ? applications.join(', ')
                : 'Light Duty Vehicles (Ford, Toyota, Honda, Chevrolet)';
                
        } catch (error) {
            return 'Light Duty Vehicles (Ford, Toyota, Honda, Chevrolet)';
        }
    }
};
